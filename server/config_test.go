...
func TestLoadConfig_DefaultPort(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NotNil(t, config)
	assert.Equal(t, 8081, config.Port) // Should be default
}
...
func TestLoadConfig_CSPHeaderFromEnvVar(t *testing.T) {
	...
	t.Setenv("CSP_HEADER", "default-src 'self'")

	config, err := LoadConfig()
	assert.NoError(t, err)
	assert.NotNil(t, config)
	assert.Equal(t, "default-src 'self'", config.CSPHeader)
}

func TestLoadConfig_HSTSMaxAgeFromEnvVar(t *testing.T) {
	tempDir := t.TempDir()
	originalDir, _ := os.Getwd()
	defer os.Chdir(originalDir)
	err := os.Chdir(tempDir)
...
func TestLoadConfig_InvalidHSTSMaxAgeEnvVar(t *testing.T) {
	tempDir := t.TempDir()
	originalDir, _ := os.Getwd()
	defer os.Chdir(originalDir)
	err := os.Chdir(tempDir)
...
func TestLoadConfig_DefaultCSPAndHSTS(t *testing.T) {
	tempDir := t.TempDir()
	originalDir, _ := os.Getwd()
	defer os.Chdir(originalDir)
	err := os.Chdir(tempDir)
	...
	assert.NoError(t, err)
	assert.NotNil(t, config)
	assert.Equal(t, "", config.CSPHeader)
	assert.Equal(t, 0, config.HSTSMaxAge)
}
